...
// Paste your own UID here — the same one you used in the SQL migration
        ...
        <div className="space-y-2">
          {listings.map((l) => (
            <div key={l.id} className="flex items-center justify-between bg-white border border-indigo-950/10 rounded p-3 text-sm">
              ...
              <button onClick={() => deleteListing(l.id)} className="text-red-600 underline text-xs">
                Delete
              </button>
            </div>
          ))}
        ...
        <div className="space-y-2">
          {users.map((u) => (
            <div key={u.id} className="flex items-center justify-between bg-white border border-indigo-950/10 rounded p-3 text-sm">
              ...
              <button onClick={() => toggleBan(u.id, u.is_banned)} className="text-xs underline">
